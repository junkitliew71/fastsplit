...
export const roundingLabels = /\b(?:bill\s*rounding|round(?:ing|\s*off)?|roundoff|rounding\s*adjustment|adjustment|pembundaran)\b/i;
...
export const paymentLabels = /\b(?:cash|tunai|card|credit\s*card|debit\s*card|visa|mastercard|amex|mydebit|duitnow|qr(?:\s*pay(?:ment)?)?|touch\s*'?n?\s*go|tng(?:\s*ewallet)?|grabpay|boost|shopeepay|mae|e-?wallet|cash\s*(?:received|tendered)|tender(?:ed)?|change|balance|outstanding|baki)\b|现金|現金|找零/i;
...
export const metadataLabels = /\b(?:receipt|bill|invoice|inv|order|queue|transaction|trans|ref)\s*(?:no|number|#)?\b|\b(?:cashier|server|staff|waiter|table|pax|persons?|people|covers?|guests?|date|time|address|malaysia|tel|phone|hp|whatsapp|reg(?:istration)?|business\s*registration|company|ssm|sst|gst)\b/i;
export const footerLabels = /\b(?:thank\s*you|thank\s*you\s*for\s*visiting|don'?t\s*forget\s*to\s*rate|hope\s*to\s*see\s*you|scan\s*qr|qr\s*code|terima\s*kasih|please\s*come\s*again|sila\s*datang\s*lagi|no\s*refund|goods\s*sold|follow\s*us|facebook|instagram|wifi|member\s*(?:id|points?)|loyalty|points?\s*(?:earned|balance))\b/i;
