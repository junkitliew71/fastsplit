...
function item(name: string, quantity: number, total: number): ReceiptItem | null {
  const clean = name.replace(/\s+/g, ' ').replace(/^[^\p{L}\p{N}]+/u, '').replace(/^\d{4,14}\s+/, '').trim().slice(0, 100);
...
function quantityAndName(line: string, values: number[]) {
  ...
  const quantityFirst = !explicit && values.length > 1 ? line.match(/^\s*(\d{1,2})\s+(?=\p{L})/u) : null;
